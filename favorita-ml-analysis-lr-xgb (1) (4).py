...
# ### 7.3.3  Create Seasonal Features
# 
...
hol_df = hol_df.drop(['Day', 'Month', 'Year'], axis=1)
...
hol_df = hol_df.set_index('date').to_period('D')
...
holidays = (hol_df.query("locale in ['National', 'Regional']").loc['2017':'2017-08-15', ['description']])
...
# let's take a closer look at these dates on the deseasonalised plot 
holidays.index
...
ax.set_title('National and Regional Holidays 2017')


# ### 7.3.4 Create Holiday Features 
...
X_holidays = pd.get_dummies(holidays) # convert holiday categorical data into dummy or indicator variables 
...
#fit the seasonal model with holiday features added on 
...
# ## measure performance with added holiday features?!?!?
...
store_sales.drop(columns=['id', 'onpromotion'], inplace=True)
...
df_test = pd.read_csv(r"C:\Users\Krupa\Downloads\store-sales-time-series-forecasting\test.csv",
...
# 
# - serial dependence: where the observations in a series are correlated with previous observations at differnt lags (put simply, the value of a variable at a certain point in time is dependent on its previous value).
...
# 
# To confirm if the deseasonalised series shows any sign of cyclic patterns, let's isolate cyclic behaviour using a moving-average plot.
...
def lagplot(x, y=None, lag=0, standardize=False, ax=None, **kwargs):
...
# 
# The correlogram suggests that the first and eighth lag could be significant. The lag plots reflect the correlogram and identify a mostly linear relationship. 
# 
# To improve forecasting, let's consider looking at leading indicators. They can provide "advance notice" of changes in the target. A useful leading indicator could be the "onpromotion" series. Since the company itself decides when to do a promotion, there's no worry about "lookahead leakage". For example, we could use Tuesday's onpromotion value to forecast sales on Monday. 
# 
...
def plot_leads(x, y=None, lead=3, nrows=2, lagplot_kwargs={}, **kwargs):
    import math 
    kwargs.setdefault('nrows', nrows)
...
# 
# For this reason, we will carry out the following steps to build our hybrid model: -  
# 
# 1. use linear regression to extrapolate the trend
# 2. transform the target to remove the trend 
# 3. then apply XGBoost to the de-trended residuals
# 
...
# create a Boosted Hybrid class to create and train LR and XGBoost models

class BoostedHybrid:
    def __init__(self, model_1, model_2): #model 1 - LR to extrapolate trend using original series, model 2 - XGBoost to make predictions based on the residual series
...
# ### 8.2 Define and add fit method to Boosted Hybrid Class 

# In[233]:


# define and add fit method to Boosted Hybrid class
...
# ### 8.3 Define and add predict method to Boosted Hybrid Class 
...
# Create and train Linear Regression and XGBRegressor models using Boosted Hybrid Class
...
# A RMSLE value between 0.2 and 0.5 is considered to be a good score for an accurate model. Based on the various models considered and explored, the Linear Regression model with the onpromotion time series aa  feature provided the most accurate results. For our submission, we will apply this ML model.  

# In[ ]:


# make predictions for store sales using linear regression and lags of "onpromotion" time series as a feature ... 


# In[266]:


# step 1 - load and prep. test data 
...
# create lag of on promotion series as a feature

family_sales = (df_test.groupby(['family', 'date']).mean().unstack('family').loc[:, ['onpromotion']])
...
supply_sales = family_sales.loc(axis=1)[:,]
supply_sales
...
onpromotion = supply_sales.loc[:, 'onpromotion'].squeeze()
#.rename('onpromotion')
